...
<html>
    <head>
        <title>Bend CHE - Admin - Ažuriranje nastupa</title>
        <meta charset="utf-8">                
        ...
        <div>
            <main>				                
                ...
                <div id="main-content-wrapper">
                    <div class="container">
                        <h2 class="text-center page-title dashboard-title">Ažuriranje nastupa</h2>
                        <div class="row">
                            <div class="col-lg-3">
                            ...
                            <div class="col-lg-9 text-center">
                                <?php
                                    ...
                                    $prep = $db->prepare("SELECT `event`.`id`, `event`.`description`, `event`.`date` FROM `event` WHERE `id` = ?;");
                                    $prep->execute([$id]);
                                    $res_event = $prep->fetchAll(PDO::FETCH_OBJ);
                                ?>
                                <form action="php/updateEvent.php" method="POST">
                                    <input id="event_id" type="hidden" name="event_id" value="<?php echo $res_event[0]->id; ?>" />
                                    <div class="form-group">
                                        <label for="description">opis:</label>
                                        <textarea id="description" class="form-control" rows="4" name="description" spellcheck="false" maxlength="120" required><?php echo $res_event[0]->description; ?></textarea>
                                    </div>

                                    <div class="form-group">                                    
                                        <label for="date">datum:</label>
                                        <input type='date' name='date' class="form-control" id="modal_date" value="<?php echo $res_event[0]->date; ?>" required/>
                                    </div>
                                    <br>
                                    <div class="form-group">
                                        <button class="btn" type="submit">Izmeni brend</button>
                                    </div>
                                </form>  
                            </div>
                        </div>
